...
3. Pour retirer le code après fusion, préparer un revert du commit de fusion (`git revert -m 1 3462623`), puis ajuster les documents modifiés après cette fusion. Examiner le résultat avant de publier ; ne pas réécrire l'historique de `main`.
4. Le worktree et la branche `lab4` peuvent être retirés séparément après vérification des fichiers locaux. Leur suppression seule ne retire plus le Lab 4 de `main`.
